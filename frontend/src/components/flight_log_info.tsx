...
const FlightLogInfo = (props: FlightLogInfoProps) => {
    ...
    return <StandardModal {...otherProps}>
        <Modal.Header>
            <div style={{
                width: '100%',
                display: 'flex',
                ...
                <FontAwesomeIcon icon={faMultiply} size='2x' className='icon-button' onClick={() => {
                    otherProps.handleClose()
                }} />
            </div>
        </Modal.Header>
        <Modal.Body>
            Max Altitude: {flightlog.max_altitude}ft<br />
            Min Altitude: {flightlog.min_altitude}ft<br />
            Distance Travelled: {(flightlog.dist_travelled_meters / 1000).toFixed(2)}km
        </Modal.Body>
    </StandardModal>
}
